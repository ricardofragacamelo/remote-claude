...
describe('DiagSession', () => {
  ...
  it('restores the count and the timestamps it was persisted with', () => {
    const session = aDiagSession({
      pingCount: 7,
      lastPingedAt: new Date('2026-09-13T13:00:00.000Z'),
    });
  ...
  it('recognises its owner', () => {
    expect(aDiagSession({ ownerId: 'auth|owner' }).isOwnedBy(UserId.create('auth|owner'))).toBe(
      true,
    );
  });

  it('does not recognise anybody else', () => {
    expect(aDiagSession({ ownerId: 'auth|owner' }).isOwnedBy(UserId.create('auth|other'))).toBe(
      false,
    );
  });

  it('round-trips through its snapshot', () => {
    const original = aDiagSession({ pingCount: 3 });

    const restored = DiagSession.restore(original.snapshot());
