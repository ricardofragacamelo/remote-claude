...
describe('DrizzleSessionRepository', () => {
  ...
  afterEach(async () => {
    ...
    await connection.db.execute(sql`TRUNCATE TABLE "sessions"`);
  ...
  const build = (): DrizzleSessionRepository =>
    new DrizzleSessionRepository(connection.db, clock, log.logger);
  ...
  it('saves a session and reads back an entity, never a row', async () => {
    ...
    expect(found).toBeInstanceOf(Session);
  ...
  it('keeps two sessions apart', async () => {
    ...
    await repository.save(Session.open(id(), owner, openedAt));
    await repository.save(Session.open(id('01J0ABCDEFGHJKMNPQRSTVWXY0'), owner, openedAt));
  ...
  it('stores instants with a zone and reads them back in UTC', async () => {
    ...
    const session = Session.open(id(), owner, at);
  ...
  it('logs both queries with their parameterised SQL and a duration', async () => {
    ...
    expect(queries.map((line) => line['operation'])).toEqual(['session.save', 'session.findById']);
  ...
  it('refuses a ping count below zero, at the database level', async () => {
    await expect(
      connection.db.execute(
        sql`INSERT INTO "sessions" ("id", "owner_id", "opened_at", "last_pinged_at", "ping_count")
