...
| `acks/connection-ready` | resposta do handshake — é `ack`, não `event`, conforme o [05](../../architecture/shared/05-websocket-protocol.md#handshake) |
| `events/error` | envelope de erro |
...
Mesmo schema → Dart em `mobile/lib/core/network/contracts/protocol.g.dart`. O Flutter está fora do workspace
...
passa no guard), S-04 (campo desconhecido é aceito), S-80 (campo obrigatório ausente é
rejeitado), S-81 (geração idempotente), S-82 (`kind` fora do enum reprova a geração).
