...
As raízes permitidas vêm de **arquivo de configuração** validado por schema — não de variável de
ambiente. O enunciado original dizia o contrário; quem manda é a [D-02](decisions.md#d-02--onde-mora-a-allowlist),
que é posterior e está propagada em dois documentos normativos. A variável de ambiente diz apenas
**onde** o arquivo está.

Lista vazia, caminho relativo ou raiz inexistente **impedem o processo de subir**, com mensagem
dizendo qual entrada do arquivo e o que se esperava —
[configuração](../../architecture/shared/07-repository-layout.md#configuração-e-segredo). A recarga
é explícita, nunca um watch.
